...
public class NumericExtrapol3EvalQuad1DOptimizeStepTest {

	private static final ExprBuilder B = ExprBuilder.INSTANCE;
	private static final double	PREC = 1e-7; // 1e-8.. fail
	private static final double	STEP_PREC = 1e-4; // if too small => rounding error!
	private static final VarDef xDef = new VarDef("x");
	private static final Expr x = xDef.expr;
	
	private final NumericExtrapol3EvalQuad1DOptimizeStep sut = 
			new NumericExtrapol3EvalQuad1DOptimizeStep(xDef, STEP_PREC);
	...
	public void testOptimStep_PureNumeric_NotQuad() {
		...
		Expr expr = B.sum( // 2 x^4 + 3 x^3 + 4 x^2 + 5 x + 2.16
				...
				B.lit(2.16));
		// when-then
		doTestOptimStep_PureNumeric_NotQuad(expr, 30,
				-0.8285177655516776, -6.05094548e-4); // TODO use real value from math solver
	}
	...
	public void testOptimStep2_PureNumeric_NotQuad() {
		...
		// when-then
		doTestOptimStep_PureNumeric_NotQuad(expr, 30, 
				-0.50850724272, 0.48410645091698534); // TODO use real value from math solver
	}
	...
	private void doTestOptimStep_PureNumeric_NotQuad(Expr expr, 
		...
		int finishedStep = -1;
		double finishConvergencePrecision = 1e-9;
